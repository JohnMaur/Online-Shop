...
//   const [nameExists, setNameExists] = useState(false); // State to manage supplier name existence status
...
//   // Function to check if the supplier name already exists
//   const checkNameExists = async (name) => {
//     const response = await fetch(`http://localhost:3000/api/supplier-exists?name=${name}`);
//     const data = await response.json();
//     setNameExists(data.exists); // Update the nameExists state based on the response
//   };

//   const handleSubmit = async () => {
//     try {
//       // Check if the supplier name already exists before submitting the form
//       const response = await fetch(`http://localhost:3000/api/supplier-exists?name=${formData.name}`);
//       const data = await response.json();
//       if (data.exists) {
//         setNameExists(true);
//         return;
...
//       const addResponse = await fetch('http://localhost:3000/api/add-supplier', {
//         method: 'POST',
...
//         });
//         setNameExists(false); // Reset nameExists state after successful submission
//       } else {
...
//             onBlur={() => checkNameExists(formData.name)} // Check if name exists when focus leaves input field
//           />
//           {nameExists && (
//             <Alert message="Supplier name already exists." type="error" showIcon />
//           )}
//         </Form.Item>
