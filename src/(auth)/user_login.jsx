    ...
    try {
      ...
      if (error.response?.status === 409) {
        MySwal.fire({
          ...
          text: 'Your account is currently active on another device. To continue here, please confirm your identity.',
          showCancelButton: true,
          confirmButtonText: 'Login on this device',
          ...
          if (result.isConfirmed) {
            ...
            if (otpSendRes.status === 200) {
              const { value: otpInput } = await MySwal.fire({
                title: 'Enter OTP',
                input: 'text',
                inputPlaceholder: 'OTP sent to your Gmail',
              });
              ...
              if (otpInput) {
                try {
                  ...
                  if (otpVerifyRes.status === 200) {
                    MySwal.fire(
                      'Success',
                      'You’ve successfully logged in on this device.',
                      'success'
                    );
                  }
                } catch (verifyErr) {
                  MySwal.fire("Error", verifyErr.response?.data?.message || "Invalid OTP", "error");
                }
              }
            }
          }
