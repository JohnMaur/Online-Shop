...
// import {
//   Box, Flex, Text, Image, Stack, Select, Spinner, useToast,
...
//           <Text fontSize="2xl" fontWeight="bold" mb={4} className='mt-14'>
//             Staff {statusFilter === 'shipping' ? "Shipping" : statusFilter === 'receiving' ? "Receiving" : "Order Received"} Details
//           </Text>

//           <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} mb={4} w="200px">
...
//           ) : orderData.length > 0 ? (
//             orderData.map(order => (
//               <Box key={order._id} bg="white" p={5} borderRadius="lg" shadow="md" mb={4}>
//                 <Text fontSize="xl" mb={2}>
//                   Delivery Date: {order.shippingDate?.shippingStringDate || "N/A"}
//                 </Text>
//                 <Flex direction={{ base: 'column', md: 'row' }} gap={4}>
//                   <Box flex="1" bg="gray.50" p={4} borderRadius="lg">
//                     <Flex gap={4}>
//                       <Image src={order.imageUrl} alt="Product" boxSize="120px" objectFit="contain" />
//                       <Stack spacing={1}>
//                         <Text fontWeight="bold">{order.productName}</Text>
//                         <Text>₱{order.price}</Text>
//                         <Text>Qty: {order.quantity}</Text>
//                         <Text>Payment: {order.paymentMethod}</Text>
//                         <Text color="gray.500">Ordered: {new Date(order.orderedAt).toLocaleString()}</Text>
//                       </Stack>
//                     </Flex>
//                   </Box>
//                   <Box flex="1" bg="gray.100" p={4} borderRadius="lg">
...
//                         <Text fontWeight="bold">{order.productName}</Text>
//                         <Text>₱{order.price}</Text>
//                         <Text>Shipping fee: {order.shippingPrice}</Text>
...
const OrderTransac = () => {
  ...
  const handleOrderReceived = async (orderId) => {
    ...
    try {
      await axios.post(`https://online-shop-server-1.onrender.com/api/mark-received`, {
        orderId,
        orderReceivedDate: dayjs().format('YYYY-MM-DD'),
        staffUsername,
      });
...
// import { StaffNavBar, Header, MobileStaffNavbar } from "../layout";
// import { ShippingOrders, ReceivingOrders, ReceivedOrders, CanceledOrders } from '../../components';
// import Select from 'react-select';
...
//   const fetchOrders = () => {
//     setLoading(true);
//     setOrderData([]);  // Clear data before fetching new orders
//   };
...
//             <Spinner size="xl" />
//           ) : statusFilter === 'shipping' ? (
//             <ShippingOrders setOrderData={setOrderData} setLoading={setLoading} />
//           ) : statusFilter === 'receiving' ? (
//             <ReceivingOrders setOrderData={setOrderData} setLoading={setLoading} />
//           ) : statusFilter === 'received' ? (
//             <ReceivedOrders setOrderData={setOrderData} setLoading={setLoading} />
//           ) : (
//             <CanceledOrders setOrderData={setOrderData} setLoading={setLoading} />
//           )}
//         </Box>
//       </Box>
//     </Box>
